...
def test_tmatrix_init(frequencies):
    ...
    _expect_data_with_wrong_abcd_dims(np.ones([2, num_bins]), frequencies)
...
def _expect_error_abcd_same_type(A, B, C, D):
    with pytest.raises(
        ValueError,
        match="If using FrequencyData objects, all matrix entries "
...
def test_tmatrix_from_abcd_input_types(frequencies, A_list, A_np, A_FreqDat):
    ...
    TransmissionMatrix.from_abcd(A_list, A_list, A_list, A_list, frequencies)
...
def test_tmatrix_from_abcd_optional_frequencies(A_list, A_FreqDat):
    ...
    with pytest.raises(
        ValueError,
        match="'frequencies' must be specified if "
...
def test_tmatrix_from_abcd_creation_from_frequency_data(A_FreqDat):
    t = TransmissionMatrix.from_abcd(
        A_FreqDat, A_FreqDat, A_FreqDat, A_FreqDat
    )
...
def test_tmatrix_from_abcd_creation_from_number(A_number, frequencies):
    t = TransmissionMatrix.from_abcd(A_number, A_number, A_number, A_number)

    _compare_tmat_vs_abcd(t, A_number, A_number, A_number, A_number)
    with pytest.warns(
        UserWarning,
        match="A, B, C, and D are all scalar, will disregard",
    ):
        TransmissionMatrix.from_abcd(
            A_number, A_number, A_number, A_number, frequencies
        )


def test_tmatrix_from_abcd_creation_from_complex(
    A_complex, frequencies_number
):
    t = TransmissionMatrix.from_abcd(
        A_complex, A_complex, A_complex, A_complex, frequencies_number
    )
...
def _compare_tmat_vs_abcd(tmat, A, B, C, D):
    ...
    if isinstance(tmat, TransmissionMatrix):
        ...
        tA, tB, tC, tD = (tmat[0, 0], tmat[0, 1], tmat[1, 0], tmat[1, 1])
        assert tA == A
        assert tB == B
        assert tC == C
        assert tD == D
...
def test_tmatrix_slicing(frequencies):
    ...
    error_msg = (
        "Indexed dimensions must not exceed the ABCD "
